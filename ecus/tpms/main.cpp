...
int main() {
    ...
    if (firmwarePath.empty() || signaturePath.empty()) {
        std::cerr << "TPMS secure boot failed: firmware/signature missing" << std::endl;
        return 1;
    }
    ...
    if (trustedPublicKeyPath.empty()) {
        std::cerr << "TPMS secure boot failed: trusted key missing" << std::endl;
        return 1;
    }
    ...
    if (!boot.verifyFirmware(firmwarePath.string(), signaturePath.string(), error)) {
        std::cerr << "TPMS secure boot failed: " << error << std::endl;
        return 2;
    }

    HsmClient hsmClient("/tmp/ecu_hsm.sock");
    if (!hsmClient.connect()) {
        std::cerr << "TPMS failed to connect to HSM: " << hsmClient.lastError() << std::endl;
        return 3;
    }
    if (!hsmClient.provisionSymmetricKey("tpms_key", 16)) {
        std::cerr << "TPMS failed to provision key: " << hsmClient.lastError() << std::endl;
        return 4;
    }

    CanSocket socket("vcan0");
    if (!socket.isOpen()) {
        std::cerr << "TPMS unable to open CAN socket: " << socket.lastError() << std::endl;
        return 5;
    }

    std::uint8_t sequence = 0;
    TaskScheduler scheduler;
    scheduler.addPeriodicTask(400, [&]() {
        ...
        bool success = false;
        std::vector<uint8_t> mac = hsmClient.computeMac("tpms_key", raw, success);
        if (!success || mac.size() < 2) {
            std::cerr << "TPMS MAC failed" << std::endl;
            return;
        }

        CanFrame frame;
        if (!CanAuthenticator::createAuthenticatedFrame(0x130, message.sequence, message.messageType, {message.data, message.data + 4}, {mac[0], mac[1]}, frame)) {
            std::cerr << "TPMS failed to create frame" << std::endl;
            return;
        }
        if (!socket.send(frame)) {
            std::cerr << "TPMS failed to send frame" << std::endl;
            return;
        }
        std::cout << "TPMS published tire frame seq=" << static_cast<int>(message.sequence) << std::endl;
    }, "tpms_publish");
